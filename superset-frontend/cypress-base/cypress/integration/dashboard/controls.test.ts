...
describe('Dashboard top-level controls', () => {
  ...
  xit('should allow chart level refresh', () => {
    const mapSpec = WORLD_HEALTH_CHARTS.find(({ viz }) => viz === 'world_map') as ChartSpec;
    waitForChartLoad(mapSpec).then(gridComponent => {
      ...
      cy.get(`[data-test="slice_${mapId}-menu"]`).find('[data-test="refresh-chart-menu-item"]').click({ force: true });
      ...
      cy.get('[data-test="refresh-chart-menu-item"]').should('have.class', 'ant-dropdown-menu-item-disabled');
      waitForChartLoad(mapSpec);
      cy.get('[data-test="refresh-chart-menu-item"]').should('not.have.class', 'ant-dropdown-menu-item-disabled');
    });
  });
  ...
  it('should allow dashboard level force refresh', () => {
    ...
    getChartAliasesBySpec(WORLD_HEALTH_CHARTS).then(aliases => {
      ...
      cy.get('[data-test="refresh-dashboard-menu-item"]').should('not.have.class', 'ant-dropdown-menu-item-disabled');

      cy.get('[data-test="refresh-dashboard-menu-item"]').click({
        force: true,
      });
      cy.get('[data-test="refresh-dashboard-menu-item"]').should('have.class', 'ant-dropdown-menu-item-disabled');
      ...
      cy.wait(aliases).then(xhrs => {
        xhrs.forEach(async ({ response, request }) => {
          ...
          const isCached = isLegacyResponse(responseBody) ? responseBody.is_cached : responseBody.result[0].is_cached;
    ...
    cy.get('[aria-label="more-horiz"]').click();
    cy.get('[data-test="refresh-dashboard-menu-item"]').and('not.have.class', 'ant-dropdown-menu-item-disabled');
  });
});
