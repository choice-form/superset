...
describe('Dashboard edit mode', () => {
  ...
  it('remove, and add chart flow', () => {
    ...
    cy.get('[data-test="dashboard-builder-component-pane-tabs-navigation"]').find('.ant-tabs-tab').last().click();

    // find box plot is available from list
    cy.get('[data-test="dashboard-charts-filter-search-input"]').type('Box plot');
    cy.get('[data-test="card-title"]').should('have.length', 1);

    drag('[data-test="card-title"]', 'Box plot').to('.grid-row.background--transparent:last');

    // add back to dashboard
    cy.get('[data-test="grid-container"]').find('.box_plot').should('be.visible');
