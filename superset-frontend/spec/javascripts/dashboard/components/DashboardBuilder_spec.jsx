...
import { DASHBOARD_ROOT_ID, DASHBOARD_GRID_ID } from 'src/dashboard/util/constants';
...
describe('DashboardBuilder', () => {
  ...
  beforeAll(() => {
    ...
    favStarStub = sinon.stub(dashboardStateActions, 'fetchFaveStar').returns({ type: 'mock-action' });
    activeTabsStub = sinon.stub(dashboardStateActions, 'setActiveTabs').returns({ type: 'mock-action' });
  });
  ...
  it('should render a Sticky top-level Tabs if the dashboard has tabs', () => {
    const wrapper = setup({ dashboardLayout: undoableDashboardLayoutWithTabs }, mockStoreWithTabs);
  ...
  it('should render a TabPane and DashboardGrid for first Tab', () => {
    const wrapper = setup({ dashboardLayout: undoableDashboardLayoutWithTabs });
    const parentSize = wrapper.find(ParentSize);
    const expectedCount = undoableDashboardLayoutWithTabs.present.TABS_ID.children.length;
    expect(parentSize.find(Tabs.TabPane)).toHaveLength(expectedCount);
    expect(parentSize.find(Tabs.TabPane).first().find(DashboardGrid)).toHaveLength(1);
  });
  ...
  it('should render a TabPane and DashboardGrid for second Tab', () => {
    ...
    const expectedCount = undoableDashboardLayoutWithTabs.present.TABS_ID.children.length;
    expect(parentSize.find(Tabs.TabPane)).toHaveLength(expectedCount);
    expect(parentSize.find(Tabs.TabPane).at(1).find(DashboardGrid)).toHaveLength(1);
  });
  ...
  it('should change redux state if a top-level Tab is clicked', () => {
    ...
    wrapper.find('.dashboard-component-tabs .ant-tabs .ant-tabs-tab').at(1).simulate('click');

    expect(dashboardStateActions.setDirectPathToChild).toHaveBeenCalledWith(['ROOT_ID', 'TABS_ID', 'TAB_ID2']);
  });
});
