...
function inferSqlExpressionColumn(adhocMetric) {
  if (adhocMetric.sqlExpression && sqlaAutoGeneratedMetricRegex.test(adhocMetric.sqlExpression)) {
    ...
    const indexPairedOpenParen = adhocMetric.sqlExpression.substring(0, indexFirstCloseParen).lastIndexOf('(');
...
export default class AdhocMetric {
  ...
  equals(adhocMetric) {
    return (
      ...
      (adhocMetric.column && adhocMetric.column.column_name) === (this.column && this.column.column_name)
    );
  }
