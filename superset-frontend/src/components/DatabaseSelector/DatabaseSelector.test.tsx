...
test('Should schema select display options', async () => {
  ...
  expect(await screen.findByRole('option', { name: 'information_schema' })).toBeInTheDocument();
});
...
test('Sends the correct schema when changing the schema', async () => {
  ...
  await waitFor(() => expect(props.onSchemaChange).toHaveBeenCalledWith('information_schema'));
});
