...
test('Rendering TooltipContent correctly - no timestep', () => {
  ...
  expect(screen.getByTestId('tooltip-content')?.textContent).toBe('Loaded from cache. Click to force-refresh');
});
...
test('Rendering TooltipContent correctly - with timestep', () => {
  ...
  expect(screen.getByTestId('tooltip-content')?.textContent).toBe(
    `Loaded data cached ${moment.utc('01-01-2000').fromNow()}. Click to force-refresh`,
  );
});
