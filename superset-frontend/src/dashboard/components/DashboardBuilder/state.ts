...
export const useNativeFilters = () => {
  ...
  const [dashboardFiltersOpen, setDashboardFiltersOpen] = useState(getUrlParam(URL_PARAMS.showFilters) ?? true);
  const showNativeFilters = useSelector<RootState, boolean>(state => state.dashboardInfo.metadata?.show_native_filters);
  ...
  const showDashboard =
    ...
    !(
      ...
      requiredFirstFilter.find(({ id }) => dataMask[id]?.filterState?.value === undefined)
    );
  ...
  useEffect(() => {
    if (filterValues.length === 0 && dashboardFiltersOpen && nativeFiltersEnabled) {
