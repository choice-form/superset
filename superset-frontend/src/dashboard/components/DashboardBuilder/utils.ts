...
export const getRootLevelTabsComponent = (dashboardLayout: DashboardLayout) => {
  ...
  return rootChildId === DASHBOARD_GRID_ID ? dashboardLayout[DASHBOARD_ROOT_ID] : dashboardLayout[rootChildId];
};
...
export const shouldFocusTabs = (
  ...
  event.target.className === 'ant-tabs-nav-wrap' || container.contains(event.target);

export const getRootLevelTabIndex = (dashboardLayout: DashboardLayout, directPathToChild: string[]): number =>
  Math.max(
    0,
