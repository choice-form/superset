 ...
 * under the License.
 */
import { DataRecord } from 'src/core';
import { ColorFormatters, COMPARATOR, ConditionalFormattingConfig, MULTIPLE_VALUE_COMPARATORS } from '../types';

export const round = (num: number, precision = 0) =>
  Number(`${Math.round(Number(`${num}e+${precision}`))}e-${precision}`);

export const rgbToRgba = (rgb: string, alpha: number) => rgb.replace(/rgb/i, 'rgba').replace(/\)/i, `,${alpha})`);
...
export const getOpacity = (
  ...
  extremeValue === cutoffPoint
    ? maxOpacity
    : round(
        Math.abs(((maxOpacity - minOpacity) / (extremeValue - cutoffPoint)) * (value - cutoffPoint)) + minOpacity,
        2,
      );

export const getColorFunction = (
  { operator, targetValue, targetValueLeft, targetValueRight, colorScheme }: ConditionalFormattingConfig,
  columnValues: number[],
) => {
  let minOpacity = MIN_OPACITY_BOUNDED;
  const maxOpacity = MAX_OPACITY;

  let comparatorFunction: (value: number, allValues: number[]) => false | { cutoffValue: number; extremeValue: number };
  ...
  switch (operator) {
    case COMPARATOR.NONE:
      ...
      comparatorFunction = (value: number, allValues: number[]) => {
        ...
        return value >= cutoffValue && value <= extremeValue ? { cutoffValue, extremeValue } : false;
      };
      break;
    case COMPARATOR.GREATER_THAN:
      comparatorFunction = (value: number, allValues: number[]) =>
        value > targetValue! ? { cutoffValue: targetValue!, extremeValue: Math.max(...allValues) } : false;
      break;
    case COMPARATOR.LESS_THAN:
      comparatorFunction = (value: number, allValues: number[]) =>
        value < targetValue! ? { cutoffValue: targetValue!, extremeValue: Math.min(...allValues) } : false;
      break;
...
export const getColorFormatters = (columnConfig: ConditionalFormattingConfig[] | undefined, data: DataRecord[]) =>
