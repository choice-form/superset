...
import { NULL_STRING, TIMESERIES_CONSTANTS } from 'src/visualizations/ECharts/constants';
  ...
  return Object.keys(rows[0])
    ...
    .map(key => ({
      ...
      data: rows.map((row, idx) => {
        const isNextToDefinedValue = isDefined(rows[idx - 1]?.[key]) || isDefined(rows[idx + 1]?.[key]);
        return [
          row.__timestamp,
